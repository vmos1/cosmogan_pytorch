...
#             grads=torch.autograd.grad(outputs=output,inputs=real_cpu,grad_outputs=torch.ones_like(output),allow_unused=False,create_graph=True)[0]
...
#                 f_save_checkpoint(gdict,epoch,iters,best_chi1,best_chi2,netG,netD,optimizerG,optimizerD,save_loc=save_dir+'/models/checkpoint_last.tar')  
            ...
            if (checkpoint and (epoch > 1) and (gpu==0)): # Choose best models by metric
            ...
            if ((iters % gdict['checkpoint_size'] == 0) or ((epoch == epochs-1) and (count == len(dataloader)-1) and (gpu==0))):
                netG.eval()
                with torch.no_grad():
                    fake = netG(fixed_noise).detach().cpu()
...
#     dataloader = DataLoader(dataset,batch_size=gdict['batchsize'],num_workers=0,drop_last=True,sampler=train_sampler,shuffle=False)
    ...
    print("Size of dataset for GPU %s : %s"%(world_rank,len(dataloader.dataset)))
...
#     logging.info(gdict)
    ...
    f_train_loop(dataloader,metrics_df,gdict,world_rank)
