...
def f_get_img_samples(ip_arr,gdict,rank=0,num_ranks=1):
    '''
...
def f_setup(gdict,args,metrics_df,log):
    ''' 
...
def f_train_loop(gan_model,Dset,metrics_df,gdict,fixed_noise,fixed_cosm_params):
    ...
    for epoch in range(start_epoch,epochs):
        ...
        for count, data in enumerate(Dset.train_dataloader):
            ...
            # print("Test",noise.shape,fake_cosm_params.shape)
            ...
            if torch.isnan(errD).any(): ## Check nans in D 
            ...
            if torch.isnan(errG).any(): ## Check nans in G
                logging.info(errG)
                print(errG)
                raise SystemError
