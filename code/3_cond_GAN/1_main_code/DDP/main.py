...
class Dataset:
    def __init__(self,gdict):
        ...
        for count,sigma in enumerate(gdict['sigma_list']):
            ...
            x=np.load(fname,mmap_mode='r')[:gdict['num_imgs']].transpose(0,1,2,3) ## Mod for 3D
        ...
        def f_compute_summary_stats(idx1=-50,idx2=None):
            ...
            with torch.no_grad():
                ...
                for count,sigma in enumerate(gdict['sigma_list']):
                    ...
                    val_img=np.load(ip_fname,mmap_mode='r')[idx1:idx2].transpose(0,1,2,3).copy() ## Mod for 3D
...
def f_train_loop(gan_model,Dset,metrics_df,gdict,fixed_noise,fixed_cosm_params):
    ...
    for epoch in range(start_epoch,epochs):
        ...
        for count, data in enumerate(Dset.train_dataloader):
            ...
            noise = torch.randn(b_size, 1, 1, nz, device=device) ### Mod for 3D
...
#     gdict['num_imgs']=200
    ...
    fixed_noise = torch.randn(gdict['op_size'], 1, 1, gdict['nz'], device=gdict['device']) #Latent vectors to view G progress    # Mod for 3D
