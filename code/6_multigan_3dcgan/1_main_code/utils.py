...
def f_save_checkpoint(gdict,epoch,iters,best_chi1,best_chi2,netG,netDlist,optimizerG,optimizerDlist,save_loc):
    ''' Checkpoint model '''
    
    dict1={'epoch':epoch,'iters':iters,'best_chi1':best_chi1,'best_chi2':best_chi2}
    
    if gdict['multi-gpu']: ## Dataparallel
        dict1['G_state']=netG.module.state_dict()
        dict1['optimizerG_state_dict']=optimizerG.state_dict()
        for i in range(len(netDlist)):
            dict1['D_state_{0}'.format(i)]=netDlist[i].module.state_dict()
            dict1['optimizerD_state_dict_{0}'.format(i)]=optimizerDlist[i].state_dict()
    else: 
        dict1['G_state']=netG.state_dict()
        dict1['optimizerG_state_dict']=optimizerG.state_dict()
        for i in range(len(netDlist)):
            dict1['D_state_{0}'.format(i)]=netDlist[i].state_dict()
            dict1['optimizerD_state_dict_{0}'.format(i)]=optimizerDlist[i].state_dict()
    
    # print(dict1.keys(),len(netDlist))
    torch.save(dict1,save_loc)


def f_load_checkpoint(ip_fname,netG,netDlist,optimizerG,optimizerDlist,gdict):
    ...
    if gdict['multi-gpu']:
        netG.module.load_state_dict(checkpoint['G_state'])
        for i,netD in enumerate(netDlist): netD.module.load_state_dict(checkpoint['D_state_{0}'.format(i)])
    else:
        netG.load_state_dict(checkpoint['G_state'])
        for i,netD in enumerate(netDlist): netD.load_state_dict(checkpoint['D_state_{0}'.format(i)])
    
    for i,optimizerD in enumerate(optimizerDlist): optimizerD.load_state_dict(checkpoint['optimizerD_state_dict_{0}'.format(i)])
    optimizerG.load_state_dict(checkpoint['optimizerG_state_dict'])
    ...
    netG.train()
    for i in range(len(netDlist)): netDlist[i].train()
    
    return iters,epoch,best_chi1,best_chi2,netDlist,optimizerDlist,netG,optimizerG
